...
import {
  ...
  createPage,
  navigateToPage,
} from "./utils/browser/browser";
import { BrowserError, handleError } from "./utils/error/error";
import {
  applyRateLimitDelay,
  getPageHTML,
  getPageInfo,
...
import { parseProfile, parseStats, parseTweets } from "./utils/parser/parser";
...
const collectTweetsFromPage = async (
  ...
  while (!shouldStopCollection(state, postsLimit)) {
    try {
      ...
      if (shouldStopCollection(state, postsLimit)) {
        break;
      }
